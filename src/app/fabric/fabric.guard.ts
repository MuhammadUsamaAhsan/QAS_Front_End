...
export class FabricGuard implements CanActivate {
  rights:any=[];
  modules:any=[];
  constructor(public auth:AuthenticationService, public router: Router) {}
    canActivate(): boolean {
      ...
      //this.rights=[2,3,4,5,6,7,8,9]
      this.rights=JSON.parse("[" +  this.rights + "]");


      if (this.auth.isAuthenticated() && this.rights.includes(this.modules.find(x=>x.module == "Fabric").id)) {
